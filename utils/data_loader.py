    ...
    try:
        ...
        for col in date_cols_cust:
            if col in df_customers.columns:
                df_customers[col] = pd.to_datetime(df[col], errors='coerce')

        numeric_cols_cust = ['total_spent', 'orders_count']
        for col in numeric_cols_cust:
            if col in df_customers.columns:
                df_customers[col] = pd.to_numeric(df[col], errors='coerce')
...
def advanced_clean_referring_site(url):
    ...
    try:
        ...
        # Generic domain extraction from parsed domain
        ...
        # print(f"Error cleaning URL '{url}': {e}") # Uncomment for debugging if needed
        return "Other (Error)"
...
def calculate_rfm(df_orders: pd.DataFrame, df_customers_master: pd.DataFrame) -> pd.DataFrame:
    ...
    # The 'duplicates='drop'' argument is crucial to handle cases where there aren't enough unique values for 5 distinct quantiles.
    ...
    # Fill NaN values for customers not found in order data (those who might have no orders)
    if 'Recency' in df_customers_master_with_rfm.columns:
        # Assign a high recency (e.g., max_recency + 30 days) to customers with no orders to push them to lowest R-score
        ...
        df_customers_master_with_rfm['Recency'] = 0 # Fallback if Recency column isn't even created
    ...
    # Fill scores with 1 for customers with no orders (lowest score for R, F, M)
    ...
    df_customers_master_with_rfm['RFM_Score'] = df_customers_master_with_rfm['RFM_Score'].fillna('111') # Lowest RFM Score (R1F1M1)
