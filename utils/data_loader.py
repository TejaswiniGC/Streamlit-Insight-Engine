...
def load_returned_products_data():
    ...
    try:
        creds_json_str = st.secrets.connections.gsheets_returns.token
        ...
        # Rename columns to match expected names in your application logic
        required_renames = {
            'Date': 'return_date',
            'Product Name': 'product_name',
            'Quantity': 'returned_quantity',
            'Comments': 'return_comments' # <-- ADDED THIS LINE
        }

        # Check if the columns exist before renaming to prevent KeyErrors
        ...
        final_required_columns = ['return_date', 'product_name', 'returned_quantity', 'return_comments'] # <-- ADDED 'return_comments' here too for final check
        ...
        # st.success(f"Successfully loaded and processed {len(df_returns)} rows from '{RETURNS_WORKSHEET_NAME}'.")
...
def load_customer_data():
    ...
    try:
        ...
        if 'orders_count' in df_customers.columns:
         ...
         # st.success(f"Customer data loaded successfully from BigQuery! Loaded {len(df_customers)} rows.")
        return df_customers
    except Exception as e:
...
def load_ecommerce_data():
    ...
    try:
        ...
        st.info("Also, verify your Google Cloud authentication (`gcloud auth application-default login`) and BigQuery permissions for the specified table.")
        return pd.DataFrame()
